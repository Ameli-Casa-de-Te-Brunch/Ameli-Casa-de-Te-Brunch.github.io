...
class TestConstruirPreviewYProduccion(SandboxConstruirTestCase):
    ...
    def test_informacion_servicio_se_incluye_si_hay_texto(self):
        self.escribir_config(config_valido(
    ...
    def test_tres_secciones_nuevas_se_omiten_si_null(self):
        ...
        frecuentes -- pedidas para el cierre de etapa (2026-09-11), sin
        texto real todavía: no se inventó contenido, así que con la
        config por defecto (null) ninguna de las tres debe aparecer."""
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        sin_comentarios = re.sub(r"<!--.*?-->", "", contenido, flags=re.S)
        self.assertNotIn("Pastelería y pedidos especiales", sin_comentarios)
        self.assertNotIn("La experiencia", sin_comentarios)
    ...
    def test_tres_secciones_nuevas_se_incluyen_con_texto_real(self):
        self.escribir_config(config_valido(
            pasteleria_pedidos_texto="Texto real de pastelería.",
            la_experiencia_texto="Texto real de la experiencia.",
            preguntas_frecuentes_texto="Texto real de preguntas frecuentes."))
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        self.assertIn("Texto real de pastelería.", contenido)
        self.assertIn("Texto real de la experiencia.", contenido)
        self.assertIn("Texto real de preguntas frecuentes.", contenido)
        self.assertIn('id="pasteleria-pedidos"', contenido)
        self.assertIn('id="la-experiencia"', contenido)
        self.assertIn('id="preguntas-frecuentes"', contenido)

    def test_contacto_es_alias_de_encontranos(self):
        """"Contacto" no duplica Encontranos -- es el mismo bloque con
        un segundo id ancla, ver comentario en el template."""
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        self.assertIn('id="contacto"', contenido)
        self.assertIn('id="encontranos"', contenido)
        # una única <section> de información práctica -- "contacto" es
        # un id ancla adentro de esa misma sección, no una sección
        # aparte (el conteo de "WhatsApp" no sirve acá: el footer ya
        # tiene su propio enlace de WhatsApp legítimo y aparte).
        self.assertEqual(contenido.count('class="informacion-practica"'), 1)
    ...
    def test_navegacion_principal_apunta_a_secciones_reales(self):
        """La navegación Sabores/Menú/Encontranos usa anclas reales a
        secciones que existen en el propio documento (no JS, no
        scroll-spy), y el CSS no la oculta por defecto -- tiene que
        quedar visible también en mobile, en una fila propia, sin
        depender de un botón hamburguesa."""
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        self.assertIn('aria-label="Navegación principal"', contenido)
        for ancla in ("sabores", "menu", "encontranos"):
            self.assertIn(f'href="#{ancla}"', contenido)
            self.assertRegex(contenido, rf'<section[^>]+id="{ancla}"')

        css = (build_site.DIST_PATH / "assets" / "css" / "site.css").read_text(
            encoding="utf-8"
        ).replace(" ", "").replace("\n", "")
        self.assertNotIn(".nav-principal{display:none", css)
    ...
    def test_informacion_practica_usa_semantica_address(self):
        """La lista de contacto de Encontranos vive dentro de un
        <address> real -- no un <div> genérico -- y el CSS resetea el
        font-style:italic que el navegador le aplica por defecto."""
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        bloque = re.search(
            r'<address>.*?</address>', contenido, flags=re.S
        )
        self.assertIsNotNone(bloque)
        self.assertIn('lista-editorial', bloque.group(0))
        self.assertIn('WhatsApp', bloque.group(0))

        css = (build_site.DIST_PATH / "assets" / "css" / "site.css").read_text(
            encoding="utf-8"
        ).replace(" ", "").replace("\n", "")
        self.assertIn(".informacion-practicaaddress{font-style:normal}", css)

    def test_imagenes_de_contenido_reservan_espacio_y_cargan_con_prioridad_correcta(self):
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        hero = re.search(r'<img[^>]+hero-tostada\.webp[^>]*>', contenido).group(0)
        self.assertIn('width="1080"', hero)
        self.assertIn('height="1440"', hero)
        self.assertIn('fetchpriority="high"', hero)
        self.assertIn('decoding="async"', hero)

        for nombre in (
            "sabor-torta.webp", "sabor-tarta.webp",
            "sabor-sandwich.webp", "menu-smoothies.webp",
        ):
            etiqueta = re.search(rf'<img[^>]+{re.escape(nombre)}[^>]*>', contenido).group(0)
            self.assertIn('width="1080"', etiqueta)
            self.assertIn('height="1440"', etiqueta)
            self.assertIn('loading="lazy"', etiqueta)
            self.assertIn('decoding="async"', etiqueta)

    def test_informacion_inclusiva_del_menu_esta_en_html_sin_js(self):
        build_site.construir(produccion=False)
        contenido = (build_site.DIST_PATH / "index.html").read_text(encoding="utf-8")
        for idioma in ("Español", "English", "Português", "Français", "Italiano"):
            self.assertIn(idioma, contenido)
        self.assertIn("información sobre alérgenos y preferencias alimentarias", contenido)

        sin_script = re.sub(r"<script[^>]*>.*?</script>", "", contenido, flags=re.S)
        self.assertIn("Cinco idiomas", sin_script)
        self.assertIn("alérgenos y preferencias alimentarias", sin_script)
        ...
        for nombre in ("index.html", "404.html"):
            ...
            for href in self._hrefs(contenido):
                ...
                if href.startswith("https://"):
                    ...
                    self.assertTrue(
                        href == "index.html" or href.startswith("assets/")
